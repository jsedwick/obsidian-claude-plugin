...
1. **Load memory base:**
   ...
   **Decision 024 — bridge UI bulk-resolve bypasses the LLM; bridge passively notifies on next turn.** When the user clicks Submit on the triage card, the chat-bridge frontend POSTs each row to `/api/triage/resolve` directly — those POSTs do NOT invoke an LLM turn. The bridge accumulates resolved Ns per-session and prepends a `<!--triage-update:v1 {"removed":[N,...]}-->` HTML comment to the user's next message. When you see this marker on your input, silently remove those Ns from your in-memory `triage_items[]` view — do NOT acknowledge it in your reply (it's a backchannel update, not a question). The marker is invisible in rendered markdown. You are still invoked only on Elaborate clicks and CLI input.
   ...
   - `<N> elaborate` (optionally suffixed with ` hash:<hex>`) — when `hash:<hex>` is present (bridge UI button click), look up `get_memory_base.triage_items[]` by matching `bullet_id_hash` and use that item; when absent (CLI typing), fall back to `triage_items[N-1]` (Decision 027 — 1-indexed flat list, already deduped + CWD-sorted to match the bridge UI). The hash-preferred path is robust to UI renumbering after a browser refresh + resolve cycle — UI Ns are per-render, hashes are per-bullet. Load the source session via `get_session_context` using the item's `source_session_slug`; if the body references a topic/decision/commit slug, fetch it via the appropriate tool. Reference the user-typed `<N>` in your reply, not the MCP-side array index. Explain in 2–3 sentences. **Do NOT re-render the menu** and **do NOT emit any update marker** — the original card persists in the bridge. **Non-terminal.**
...
4. **Summarize:**
   ...
   - If triage-eligible carryforward items exist (per Decision 023 in step 1), append the `## Carryforward triage` menu at the very end
